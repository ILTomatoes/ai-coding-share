/**
 * Shared helper functions for generating PPT slides.
 * Provides reusable components: titles, cards, tables, flow charts, etc.
 */
...
function addStyledTable(slide, x, y, w, headers, rows, opts = {}) {
  ...
  rows.forEach((row, idx) => {
    const bg = idx % 2 === 0 ? COLORS.DARK_ROW1 : COLORS.DARK_ROW2;
...
// ─── Code Block ───
function addCodeBlock(slide, x, y, w, h, lines, opts = {}) {
  slide.addShape("rect", {
    x, y, w, h,
...
function addSectionDivider(slide, partLabel, mainTitle, subtitle, pageNum) {
  slide.background = { color: COLORS.BG_DARK };

  // Part label (small, accent color)
...
module.exports = {
  ...
  addStyledTable, addCodeBlock, addWarningBox,
  addSectionDivider, addTextBlock,
};
